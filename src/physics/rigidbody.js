...
export class RigidBody {
  // No snapshot classification or hooks. A rigid body is debris — brass and
  // dropped magazines — and `physics` excludes the whole body list; see the note
  // on `PhysicsSystem`. An earlier revision carried the lists and the two hooks
  // here and nothing ever called them, which is the same dead weight the fork
  // audit in `84a05c4` cleared out of `player` and `Animator`.
  //
  // If debris ever gains a collider and starts stopping bullets, this is where
  // the classification goes back: the integrator (`position`, `quaternion`, the
  // two velocities, the Verlet `prev*` pair, `invInertiaWorld`, the sleep and
  // lifetime timers) rewinds, and the shape and material do not.
...
export class RigidBodyWorld {
  // No snapshot hooks here, and no classification lists either: `physics`
  // excludes `bodies` whole, so the replay gate never reaches this object. The
  // reasoning — and the wrong turn that preceded it — is on `PhysicsSystem`.
