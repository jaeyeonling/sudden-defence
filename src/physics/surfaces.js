...
export const MASK = {
  ...
  /**
   * Everything a bullet can strike — INCLUDING the player.
   *
   * The player used to be excluded here so a shot fired from the eye could not
   * hit the capsule the eye sits inside. That worked, but it also meant the only
   * way for anything to shoot the player was a bespoke code path outside the
   * ballistics solver: no penetration, no part hitboxes, no headshots, and a
   * damage model that did not match the one bots were held to.
   *
   * `physics.fireBullet({ ignore })` solves the self-hit properly by skipping
   * the shooter's own colliders, so player and bot are now the same kind of
