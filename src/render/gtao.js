...
export class Gtao {
  ...
  /**
   * Drop the accumulated history so the next frame starts from scratch.
   *
   * `taa` and `exposure` do this with a flag their shader reads, because both
   * have a "first frame" branch already. This accumulator does not: its temporal
   * pass unconditionally blends `tHistory`, so the only way to make frame N
   * behave like frame 0 is to put the history back to what frame 0 actually read
   * — a freshly allocated target, which WebGL zero-initialises. Hence a clear to
   * zero rather than a flag.
   *
   * Zero means fully occluded, which would be wrong to LOOK at. It is not meant
   * to be looked at: the criterion is reproducing boot exactly, and at boot these
   * targets are zero. The first blended frame converges out of it identically to
   * the way it does on a cold start.
   */
  reset(renderer) {
    this._flip = 0;
    if (!renderer) return;
