...
export function muzzleFlash(fx, o) {
  ...
  //
  // The cutoff came down 5 m -> 3 m and the peak 0.18 -> 0.028 when the game
  // moved indoors, and it is the enemy, not the ground, that forced both.
  // Measured on the contact frame of a real match: the player's own flash was
  // live at 31 cd, 1.8 m ahead of him, with a soldier 3.0 m further down the
  // bore. At a 5 m cutoff the window is still 0.76 there, so that soldier took
  // ~2.6 units from the flash against a sun that barely reaches him — indoors
  // `owIndirect` is 0.03, so a body in warehouse shade has almost no ambient to
  // absorb it — and he photographed as a flat cream silhouette with his camo
  // gone. You could not see the man you were shooting at, on the frame you shot
  // him. Dropping only the cutoff was not enough: at 36 cd a body at 2.4 m still
  // washed out, because the cutoff governs where the light STOPS, not how hard
  // it hits inside that radius.
  //
  // `tools/legibility.mjs` measures the result instead of eyeballing it — it
  // stages a bot at a fixed distance, photographs the frame with him and again
  // without him, and compares the same rectangle. With 3 m / 0.028 (~4.8 cd
  // peak) against a sun of 6.7:
  //
  //     dist   flash vs sun   torso contrast   saturation (unlit)   clipped
  //     1.5 m      2.1             0.52           0.23 (0.33)         11 %
  //     3 m        0.10            0.18           0.29 (0.25)          0 %
  //     6 m        0               0.27           0.16 (0.22)          0 %
  //     12 m       0               0.55           0.21 (0.24)          0 %
  //
  // So a man at engagement range takes nothing from it and reads BETTER than
  // unlit — the flash lifts the wall behind him more than it lifts him — while
  // a man in your face takes a twice-the-sun highlight that blows a tenth of his
  // torso and leaves his camo legible. That is the trade this effect is for. The
  // near pool on your own feet and walls is untouched: the window is 0.98 at 1 m.
  if (fx.lights) {
