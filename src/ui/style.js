...
/* ============================================================= match bar
   Top centre, where the compass used to be. It inherits that slot rather than
   sitting under it: with no minimap and no compass, the round score and the
   clock are the only persistent chrome up there, and burying them 45px down
   under nothing would just be a hole in the layout.

   The round pips are the reason this is not one line of text. "3-2" tells you
   the score; a row of filled and empty pips tells you how close the match is to
   over, which is the thing you actually want at a glance. */
.ow-match {
  position:absolute; left:50%; top:calc(var(--pad) * .7);
  transform: translateX(-50%);
  display:flex; flex-direction:column; align-items:center; gap: calc(var(--u) * .8);
  text-shadow: var(--sh-o1);
  /* Bottom padding is generous because the pip row has to sit INSIDE the solid
     part of the scrim, not in its lower feather — the pips are 3px tall and
     were the first thing to disappear against a pale floor. */
  padding: calc(var(--u) * 1.4) calc(var(--u) * 3.5) calc(var(--u) * 2.2);
}
/* Scrim. The compass this widget replaced carried one and left a note saying
   why: the warehouse skylight runs straight up the middle of the frame, so the
   top centre of the screen is the ONE place where the background is routinely
   blown-out cloud, and a text shadow alone gave grey glyphs on white sky.
   Feathered at both ends so it dissolves instead of terminating in a rectangle,
   and rolled off at the bottom so it does not read as a bar. */
.ow-match::before {
  content:''; position:absolute; inset:0; z-index:-1;
  background: linear-gradient(to bottom,
    rgba(3,6,9,0) 0%, rgba(3,6,9,.48) 16%, rgba(3,6,9,.48) 80%,
    rgba(3,6,9,.18) 93%, rgba(3,6,9,0) 100%);
...
.ow-match b { font-family: var(--fd); font-size: calc(23px * var(--k)); font-weight:700;
  letter-spacing:.04em; font-variant-numeric: tabular-nums; }
.ow-match .us { color: var(--friend); }
.ow-match .them { color: var(--enemy); }
.ow-match .clock {
  font-family: var(--fd); font-size: calc(17px * var(--k)); color: var(--ink);
  font-variant-numeric: tabular-nums; min-width: calc(52px * var(--k)); text-align:center;
}
/* Freeze time and round end read as amber; the last ten seconds read as danger.
   Colour is the channel, not a blink — nothing in this HUD animates on a CSS
   keyframe (see ui/index.js) and a clock is the last place to start. */
.ow-match .clock.hold { color: var(--amber); }
.ow-match .clock.urgent { color: var(--enemy); }
.ow-match .sep { width:1px; height: calc(13px * var(--k)); background: var(--hair); }
.ow-match .phase { min-width: calc(74px * var(--k)); text-align:center; }

.ow-pips { display:flex; align-items:center; gap: calc(3px * var(--k)); }
.ow-pip {
  width: calc(14px * var(--k)); height: calc(4px * var(--k));
  background: rgba(196,220,238,.30);
  box-shadow: 0 0 0 1px rgba(4,8,11,.55);
}
.ow-pip.us { background: var(--friend); }
.ow-pip.them { background: var(--enemy); }
.ow-pips .gap { width: calc(7px * var(--k)); }

/* ============================================================ scoreboard
   Held on Tab. Two columns, one per side, sharing a header row — the point of
   a scoreboard in a team game is the comparison, and two separate tables make
   you do that comparison yourself. */
.ow-sb {
  position:absolute; left:50%; top:50%; transform: translate(-50%,-50%);
  display:none; gap: calc(var(--u) * 3);
  padding: calc(var(--u) * 3);
  background: rgba(4,8,11,.82);
  box-shadow: 0 calc(6px * var(--k)) calc(30px * var(--k)) rgba(0,0,0,.55),
              inset 0 0 0 1px rgba(196,220,238,.10);
  backdrop-filter: blur(calc(3px * var(--k)));
}
.ow-sb.on { display:flex; }
.ow-sb-col { min-width: calc(276px * var(--k)); }
.ow-sb-head {
  display:flex; align-items:baseline; gap: calc(var(--u) * 1.5);
  padding-bottom: calc(var(--u) * 1.2);
  border-bottom: 1px solid var(--hair);
  font-size: calc(11px * var(--k)); letter-spacing:.2em; color: var(--ink-2);
}
.ow-sb-head .name { flex:1; }
.ow-sb-head .score {
  font-family: var(--fd); font-size: calc(21px * var(--k)); font-weight:700;
  font-variant-numeric: tabular-nums;
}
.ow-sb-col.us .name, .ow-sb-col.us .score { color: var(--friend); }
.ow-sb-col.them .name, .ow-sb-col.them .score { color: var(--enemy); }

.ow-sb-cols, .ow-sb-row {
  display:grid; grid-template-columns: 1fr calc(38px * var(--k)) calc(38px * var(--k)) calc(52px * var(--k));
  gap: calc(var(--u) * .8); align-items:center;
}
.ow-sb-cols {
  font-size: calc(9.5px * var(--k)); letter-spacing:.2em; color: var(--ink-3);
  padding: calc(var(--u) * 1.2) 0 calc(var(--u) * .6);
}
.ow-sb-row {
  font-size: calc(12.5px * var(--k)); color: var(--ink); letter-spacing:.06em;
  padding: calc(var(--u) * .55) 0;
  font-variant-numeric: tabular-nums;
}
/* Dead is stated by dimming the whole row, not by a skull in a column: at a
   glance you want to count who is left, and a column you have to read is not a
   glance. */
.ow-sb-row.down { opacity:.34; }
.ow-sb-row.you { box-shadow: inset calc(2px * var(--k)) 0 0 var(--amber); padding-left: calc(var(--u) * .9); }
.ow-sb-row .n { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.ow-sb-row .v { text-align:right; color: var(--ink-2); }
.ow-sb-row .v.k { color: var(--ink); }

/* ============================================================== spectate
   One line, low centre. Deliberately not a panel: you are watching the game,
   and the game is behind this text. */
.ow-spec {
  position:absolute; left:50%; bottom:calc(var(--pad) * 1.6);
  transform: translateX(-50%); display:none;
  flex-direction:column; align-items:center; gap: calc(var(--u) * .7);
  text-shadow: var(--sh-o1);
}
.ow-spec.on { display:flex; }
.ow-spec .dead {
  font-size: calc(12px * var(--k)); letter-spacing:.32em; color: var(--enemy);
}
.ow-spec .who {
  font-family: var(--fd); font-size: calc(20px * var(--k)); letter-spacing:.08em;
  color: var(--ink);
}
.ow-spec .hint { font-size: calc(10px * var(--k)); letter-spacing:.2em; color: var(--ink-3); }
