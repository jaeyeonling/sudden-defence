...
export class FxSystem {
  ...
  async init(ctx) {
    ...
    // Last muzzle transform pushed by whoever owns a viewmodel. Zero means
    // "nobody is holding a weapon" and the staged flash falls back to an
    // eye-relative offset. See setMuzzleTransform().
  ...
  /**
   * Make every transient pool DRAWABLE for one warm frame, then empty it again.
   *
   * `prewarmMaterials()` above compiles these five materials at boot and the
   * compile is real — `renderer.info.programs` grows, and the cache key matches
   * what the real draw asks for. It was not enough, and the measurement that says
   * so is worth keeping:
   *
   *   f181  249.5 ms   world 250.9 (x6)  seq[0.2 0 0.1 0.1 0.1 246.8]
   *         progDelta 0-1 · geoDelta 0 · texDelta 0
   *         became visible: fx-particles-lit, fx-particles-additive, fx-decals
   *
   * A quarter of a second inside ONE pass — the sixth render of the world scene,
   * the forward pass into the MSAA HDR target — with the other five world passes
   * at a tenth of a millisecond each, no new program, no new buffer, no new
   * texture, and three fx meshes becoming visible on precisely that frame. Frame
   * 181 is where the round leaves `freeze` and the trigger unlocks, so it is the
   * first frame that particles have ever existed.
   *
   * The cost is the driver, not three.js. ANGLE builds a Metal pipeline per
   * combination of shader and render-target format, and translates the shader when
   * it does; three counts WebGLProgram objects and knows nothing about any of it.
   * A compiled program that has never DRAWN into the HDR target is, from the
   * driver's side, cold.
   *
   * Which is also why `?warmhidden=1` bought nothing: it un-hides every
   * boot-hidden mesh in both scenes and compiles them, but an empty pool draws
   * zero primitives, and zero primitives build no pipeline. The stall did not move.
   *
   * So this does not spawn anything — it lifts each pool's draw count to one and
   * lets `prewarm`'s `warmDraw()` render a single degenerate instance of zeroed
   * attribute data into the real targets. That is enough for the driver and it
   * keeps the hook out of the particle bookkeeping entirely: no `emit()`, so no
   * `rng` draw, no cursor movement, no `expireAt` in the future, nothing to
   * unwind but two numbers per pool. The instance buffers were allocated at boot,
   * so no geometry grows either.
   *
   * @returns {{ok: boolean, pools: number, restore: () => void}}
   */
  prewarmTransients() {
    const saved = [];
    const arm = (mesh, geo) => {
      if (!mesh) return;
      // InstancedMesh counts through `mesh.count`; the particle pools count
      // through `geometry.instanceCount`; the decal mesh is a plain Mesh drawing a
      // span of a preallocated buffer. Three different knobs, one intent.
      const g = geo ?? mesh.geometry;
      saved.push({
        mesh,
        geo: g,
        ...
        drawStart: g?.drawRange?.start ?? 0,
        drawCount: g?.drawRange?.count,
      });
      if (typeof mesh.count === 'number') mesh.count = Math.max(1, mesh.count);
      if (g && typeof g.instanceCount === 'number') g.instanceCount = Math.max(1, g.instanceCount);
      if (g?.drawRange && !(g.drawRange.count > 0)) {
        // One triangle's worth, and only if the pool is currently drawing nothing.
        // Clamped to what the buffer actually holds so this cannot ask the driver
        // to read past the end of an attribute.
        const have = g.index?.count ?? g.attributes?.position?.count ?? 0;
        g.setDrawRange(0, Math.min(3, have));
      }
      mesh.visible = true;
    };

    // Attach the viewmodel layers first, exactly as `prewarmMaterials` does.
    //
    // Without this the two view-space pools are skipped, because `_viewAttached`
    // is still false this early — and skipping them left the whole cost sitting in
    // the other pass instead of removing it: once the world forward pass was warm,
    // the stall reappeared as `view 235.9 ms (x1)` on the first frame a muzzle
    // flash rendered. Same event, same quarter second, one pass to the left.
    if (!this._viewAttached && this._viewmodelPresent()) this._attachView();

    for (const pool of [this.lit, this.add, this.motes]) arm(pool?.mesh, pool?.geometry);
    if (this._viewAttached) {
      for (const pool of [this.viewAdd, this.viewLit]) arm(pool?.mesh, pool?.geometry);
    }
    arm(this.decals?.mesh, this.decals?.geometry);
    arm(this.shells?.mesh);
    ...
    return {
      ...
      restore: () => {
        for (const s of saved) {
          ...
          if (typeof s.count === 'number') s.mesh.count = s.count;
          if (s.geo && typeof s.instanceCount === 'number') s.geo.instanceCount = s.instanceCount;
          // `setDrawRange(0, Infinity)` is three's "draw everything" sentinel and
          // restoring it as a number would silently cap the mesh forever, so put
          // back exactly what was read — including Infinity.
          if (s.geo?.drawRange && s.drawCount !== undefined) {
            s.geo.setDrawRange(s.drawStart, s.drawCount);
          }
        }
      },
    };
   ...
   */
  /**
   * Publish where the bore currently is, in world space.
   *
   * The owner of a viewmodel pushes this; fx never reaches back for it. Only
   * the staged (harness) flash consumes it — a real trigger pull carries its
   * own origin on the `weapon:fire` payload. Pass nothing to clear.
   *
   * @param {THREE.Vector3} [position] Muzzle crown in world space.
   */
  setMuzzleTransform(position) {
    if (position) this._muzzleWorld.copy(position);
    else this._muzzleWorld.set(0, 0, 0);
  }
  ...
  onActorDeath(e) {
    // Checking `.point` alone is not enough, and that is not hypothetical: this
    // ran for a whole match against a payload whose `point` was the STRING
    // 'torso' — truthy — and the first `.x` off it arrived at
    // `physics.groundHeight` as undefined. Effects are decoration. An unusable
    // payload is a reason to draw nothing, never a reason to throw inside
    // somebody else's event emit.
    if (typeof e?.point?.x !== 'number') return;
    this.now = this.ctx.time.elapsed;
    const rng = this.rng;
   ...
   *
   * The crown comes from the last pushed muzzle transform when there is one, so
   * the flash is welded to the bore rather than to a hardcoded offset from the
   * eye. The offset is only the fallback for an FX-only scene.
   */
  _stageMuzzle() {
    const cam = this.ctx.camera;
    cam.updateMatrixWorld();
    // Push, not poll: whoever owns a viewmodel calls setMuzzleTransform(). fx
    // must not know that a `weapons` subsystem exists.
    const welded = this._muzzleWorld.lengthSq() > 1e-6;
    if (welded) this._tmpA.copy(this._muzzleWorld);
    else this._tmpA.set(0.16, -0.13, -0.72).applyMatrix4(cam.matrixWorld);
