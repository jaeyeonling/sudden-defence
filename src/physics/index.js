...
export class PhysicsSystem {
  ...
  constructor() {
    ...
    this._impactResult = [];
    /** Who fired the trace currently in flight. See fireBullet(). */
    this._bulletSource = null;
    /**
     * Collider owner to skip while a trace is in flight — the shooter's own
     * hitboxes. See fireBullet() and _raycastColliders().
     */
    this._ignoreOwner = null;
    /** Optional `(source, target) => boolean` veto on damage. setDamageFilter. */
    this._damageFilter = null;
    /** Preallocated `damage:dealt` payload — one hit per round, reused. */
    this._damagePayload = {
      target: null, source: null, amount: 0, part: 'torso',
      headshot: false, team: null, killed: false, point: null,
    };
  ...
  _raycastColliders(ox, oy, oz, dx, dy, dz, best, mask, out) {
    for (let i = 0; i < this.colliders.length; i++) {
      ...
      // The shooter's own hitboxes. A trace starts at the eye, which is inside
      // the shooter's head capsule, so without this every shot is a self
      // headshot the instant PLAYER/ACTOR enter MASK.BULLET.
      if (this._ignoreOwner !== null && c.owner === this._ignoreOwner) continue;
      const t = c.shape === 'box'
   ...
   */
  /**
   * `opts.source` is whoever pulled the trigger. It is stashed for the duration
   * of the trace rather than threaded through the penetration solver, which is
   * safe because fire() is fully synchronous: every emitImpact() it causes runs
   * before this returns. Anything reacting to a hit — hitmarkers, killfeed,
   * friendly fire — needs it, because once bots shoot each other "who was hit"
   * stops implying "who landed it".
   *
   * `opts.ignore` (default: the source) is the collider owner the trace passes
   * straight through. This is what lets PLAYER and ACTOR live in MASK.BULLET at
   * all: everyone's trace begins inside their own head capsule. Pass `null`
   * explicitly if you genuinely want a trace that can hit its own shooter.
   *
   * Both flags are cleared in `finally`, so they only ever apply to the rays
   * this call issues — the penetration solver's exit probes included, which is
   * what stops a round that grazed a shoulder from measuring the shooter's own
   * torso as the material it has to punch through.
   */
  fireBullet(opts) {
    this._bulletSource = opts?.source ?? null;
    this._ignoreOwner = opts && 'ignore' in opts ? opts.ignore : this._bulletSource;
    try {
  ...
  /**
   * Install a veto on damage: `(source, target) => boolean`, where both are the
   * `owner` objects on the colliders involved. Returning false suppresses the
   * `damage:dealt` event; `bullet:impact` still fires, so the round still marks
   * the wall, sprays and cracks — it simply does not wound.
   *
   * This exists so `match` can turn off friendly fire WITHOUT physics learning
   * what a team is. The alternative was to let the victim's own damage handler
   * check the team, which works only as long as `match` happens to be
   * subscribed to `damage:dealt` before every host is — i.e. as long as nobody
   * ever changes the dependency graph. Pushing a predicate down here makes the
   * rule independent of subscription order, which is the only way it stays true.
   */
  setDamageFilter(fn) {
    this._damageFilter = typeof fn === 'function' ? fn : null;
  }
  ...
  emitImpact(px, py, pz, nx, ny, nz, dx, dy, dz, si, damage, exit, hit) {
    ...
    //
    // A ray that hits a ragdoll fills in `actor` (the dead agent) and takes
    // `part` from the RAGDOLL's bone spec — 'Head', 'UpperArmR' — rather than
    // from a hitbox's part label. Both of those leaked: `damage:dealt` fired for
    // the body on the floor, and `match` took it as the victim's most recent
    // wound, overwriting `_lastAttacker`, `_lastPart` and `_lastHeadshot` in the
    // window between the fatal round and the death record being assembled.
    //
    // `tools/botfight.mjs` reported two symptoms and named neither cause: a
    // death recorded with `part: 'Head'` and `headshot: false` (the flag below
    // is `part === 'head'`, and the bone is capitalised), and kills credited to
    // nobody. Bone names cannot come from anywhere else — both hitbox tables in
    // this codebase are lowercase part labels — so their presence in a death
    // record is proof the corpse path reached it.
    //
    ...
    if (p.actor && !exit && !hit?.ragdoll
      ...
      d.part = hit?.part ?? 'torso';
      d.headshot = hit?.part === 'head';
      d.team = p.actor.team ?? null;
      d.killed = false;
      d.point = p.point;
