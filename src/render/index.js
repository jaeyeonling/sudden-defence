...
export class RenderSystem {
  ...
  async init(ctx) {
    ...
    this.settings = {
      ...
      //
      // 0.035 came from a 120 m outdoor map where "interior" meant a shop off a
      // sunlit street and the whole job of this number was to keep that shop
      // 4-5 stops down from the facade framed in its own doorway. Under a roofed
      // depot it is not a room budget, it is the exposure of the entire game,
      // and it was costing the map its colour: measured in scene radiance, the
      // bravo spawn bay rendered at R 0.0043 / B 0.0050 with every other mesh
      // hidden it renders R 0.1975 / B 0.0236. Forty-six times darker, and at
      // that level the grade's shadowTint (+0.022 blue) is larger than the
      // albedo signal, so red paint, blue paint and yellow paint all arrived on
      // screen within 17 degrees of the same hue — which is `floorMarkings` job
      // 2, the per-team bay colour that is the ONLY asymmetry in the level.
      //
      // Attribution is clean: at x10 the bay goes R/B 0.86 -> 1.92 while the
      // same x10 on `iblDiffuse` moves it 0.86 -> 0.91, because the interior
      // gate multiplies the global budget away indoors and this floor is what
      // is left. See tools/legibility.mjs for the gate that keeps it honest.
      interiorIndirect: 0.22,
  ...
  /**
   * Put every temporal accumulator back to the state a freshly booted renderer
   * is in, so the next frame drawn is frame zero in every sense that shows.
   *
   * WHY THIS EXISTS — two separate reasons that turned out to be one piece of
   * work:
   *
   *  1. `tools/baseline.mjs` has always tried to call it. Line 87 reads
   *     `r?.resetTemporal?.() ?? r?.resetHistory?.() ?? r?.invalidateHistory?.()`
   *     and NONE of those three methods existed, so the pixel gate's "drop
   *     temporal history so accumulation starts from a known phase" step was
   *     three chained optional calls doing nothing at all. It got away with it
   *     because every capture uses a fresh page, where the accumulators are cold
   *     regardless.
   *  2. `src/core/prewarm.js` needs to draw one real frame through the full pass
   *     chain — the only thing that reaches the depth and prepass override
   *     variants, and worth 241 ms to 56 ms on the first shot of a round.
   *     Drawing it dirties exactly the state below. Restoring `frame` alone was
   *     not enough: that puts the jitter INDEX back and leaves the accumulated
   *     TEXTURES, measured as 99.7 % of `corner.png` moving, mean delta 8.8.
   *
   * The inventory is small and was taken by grep rather than from memory: only
   * this system, `taa`, `gtao` and `exposure` hold anything across frames. `ssr`
   * reads TAA's history but keeps none of its own; `contact`, `bloom`, `dof` and
   * `composite` are pure functions of the frame they run in. `_cullLights` looks
   * temporal and is not — `fade` is recomputed from distance every frame, and
   * `applied` is only ever compared against the value it just wrote.
   */
  resetTemporal() {
    this.frame = 0;
    this._firstFrame = true;
    // Redundant with `_firstFrame`, which makes the next frame overwrite this
    // from `_currVP` before anything reads it. Cleared anyway, because leaving a
    // stale view-projection in a field named "previous" is how a later refactor
    // gets this wrong.
    this._prevVP.identity();
    if (this.taa) {
      this.taa.reset();
      this.taa.index = 0; // the Halton jitter index, which is NOT `this.frame`
    }
    this.gtao?.reset?.(this.renderer);
    this.exposure?.reset?.();
    return { ok: true, taa: !!this.taa, gtao: !!this.gtao, exposure: !!this.exposure };
  }
  ...
  resize(w, h, _ctx) {
    ...
    // AO runs at its own resolution, because it is the single most expensive
    // thing in the frame and the only one that upsamples for free.
    //
    // Measured with `tools/abperf.mjs`, five pairs all agreeing: turning GTAO off
    // at `q=high` is worth 12.9 ms of a ~33 ms frame. Nothing else came close —
    // SSR 3.6, volumetrics 4.7, and TAA, motion blur and bloom were each inside
    // the noise. It is 3 slices x 8 steps per pixel plus a temporal resolve and a
    // two-pass blur, all at full render resolution, which at DPR 2 is 3024x1964.
    //
    // Halving it needs no shader change: the material patch samples the AO buffer
    // as `texture2D(owAoTex, gl_FragCoord.xy * owScreenTexel)` — a screen-space UV,
    // not a texel fetch — and `hdrTarget` already builds targets with LinearFilter,
    // so a smaller buffer is simply filtered up. The gbuffer it reads stays full
    // resolution and is likewise sampled by UV.
    const aoScale = this.q.aoScale ?? 1;
    this.gtao?.setSize(
      Math.max(1, Math.round(rw * aoScale)),
      Math.max(1, Math.round(rh * aoScale))
    );
   ...
   * through a 2D rotation — cheap enough to do per fragment. The gate keys off
   * *depth inside the footprint*: a wall's outer skin is at depth 0 while its
   * inner skin is one thickness in, so coarse boxes are enough and we never
   * need per-room geometry.
   *
   * Push, not poll: the world subsystem owns what counts as an interior, and
   * render must not know that a `world` subsystem exists. Call once after the
   * level is authored.
   *
   * @param {Array<{x:number, z:number, w:number, d:number, y0:number, y1:number}>} volumes
   *   Level-space boxes. `x`/`z` are the centre, `w`/`d` the full extent,
   *   `y0`/`y1` the vertical span in world units.
   * @param {(x:number, y:number, z:number, out:THREE.Vector3) => THREE.Vector3} [levelToWorld]
   *   Level->world transform. Omit when the level is authored in world space.
   */
  setRoomVolumes(volumes, levelToWorld) {
    ...
    // this stays correct whatever transform the caller authored against.
    let cs = 1;
    let sni = 0;
    let ox = 0;
    let oz = 0;
    if (typeof levelToWorld === 'function') {
      const o = levelToWorld(0, 0, 0, this._tmpV3);
      ox = o.x;
      oz = o.z;
      const ex = levelToWorld(1, 0, 0, this._tmpV3b);
      const c = ex.x - ox;
      const sn = ex.z - oz;
    ...
    for (const v of volumes ?? []) {
      if (!v || n >= rooms.length) break;
      rooms[n].set(v.x, v.z, v.w * 0.5, v.d * 0.5);
      roomsY[n].set(v.y0 ?? -0.8, v.y1 ?? 12, 0, 0);
      n++;
    }
  ...
  /**
   * ADS engagement, 0..1 — always 0 here.
   *
   * This game has no aim-down-sights: it is hipfire-only with a screen-space
   * crosshair, so the DoF and FOV channels that keyed off ADS stay parked at
   * their hip values. The hook is kept (rather than the reads being deleted at
   * every call site) so the post chain reads the same either way.
   */
  _readAds() {
    return 0;
  }
