...
export class Ambience {
  ...
  _shimmer(dt, now, camera) {
    ...
    if (fx.physics?.groundHeight) {
      // +1 m, not +6. Indoors a 6 m probe starts above the roof and comes back
      // with the roof slab, so the shimmer stacks on the ceiling instead of the
      // floor. One metre clears anything you could be standing on and still
      // stays under any ceiling worth walking beneath.
      const h = fx.physics.groundHeight(sx, sz, camera.position.y + 1);
