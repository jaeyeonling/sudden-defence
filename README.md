...
Thirty-five tools live in `tools/`, and twenty-five of them are gates that
`npm test` runs. Almost none of them are unit tests. They ask questions about
the *experience*, because that is what can actually break:
...
| `sight` | how much of what you are aiming at survives the frame? |
...
```bash
npm test              # all 25
npm run test:logic    # 16 that read engine state only — no pixels, no timing
...
**Deterministic spray.** A fixed seeded recoil pattern you can memorise — the
springs kick the viewmodel, never the point of impact. **Hitscan.** No travel
