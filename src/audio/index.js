...
export class AudioSystem {
  ...
  _onDamageDealt(p) {
    ...
    //
    // The hitmarker tick is UI: it means "the shot YOU just took connected", so
    // it is gated on the SOURCE. It used to be gated on the target instead —
  ...
  /**
   * Combatant identity WITHOUT `peek('player')`.
   *
   ...
   */
  _isPlayer(c) {
    return c === 'player' || c?.isPlayer === true;
  }
