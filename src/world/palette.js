...
export const PALETTE = {
  ...
  // ------------------------------------------------------------ depot paint --
  /**
   * Floor and steel paint for the warehouse.
   *
   * These exist because the depot was grey on grey: concrete floor, concrete
   * walls, concrete partitions, steel racking. It photographed as one material
   * lit three ways, and in a symmetric map that also means you cannot tell
   * which half you are standing in.
   *
   * The base generator is `plaster`, NOT `concrete_floor`, and that is the whole
   * trick. `tint` is a linear MULTIPLY on the baked albedo, so paint can never
   * come out brighter than the material underneath it — a yellow tint over
   * concrete's mid-grey bake lands at roughly 0.3/0.23/0.06 linear, which is a
   * dark ochre. Under this roof the only direct light is the skylight strip, so
   * that ochre disappeared into the floor entirely: the first version of these
   * markings was invisible in every shot. Plaster bakes much brighter, so the
   * same tint has somewhere to go.
   *
   * `surface` stays 'concrete' regardless — paint is 100 microns of enamel and
   * should sound, spark and scuff like the slab it sits on.
   */
  paint_yellow: {
    name: 'plaster',
    surface: 'concrete',
    opts: { vertexMasks: true, tint: 0xe8b41c, scale: 0.5, weather: [0.5, 0.4, 0.35, 0.5] },
  },
  /** Alpha's end of the depot. Cool, so it reads against the warm racking. */
  paint_alpha: {
    name: 'plaster',
    surface: 'concrete',
    opts: { vertexMasks: true, tint: 0x2f7ec0, scale: 0.7, weather: [0.55, 0.45, 0.4, 0.55] },
  },
  /** Bravo's end. Warm, and the only large red surface in the level. */
  paint_bravo: {
    name: 'plaster',
    surface: 'concrete',
    opts: { vertexMasks: true, tint: 0xc4402a, scale: 0.7, weather: [0.55, 0.45, 0.4, 0.55] },
  },
  /** Pallet-rack load beam. The one warm accent inside the lanes. */
  metal_orange: {
    name: 'metal_painted',
    surface: 'metal',
    opts: { vertexMasks: true, tint: 0xb0601f, scale: 0.8, weather: [0.45, 0.4, 0.4, 0.5] },
  },
