...
export class Viewmodel {
  ...
  addRecoil(pitch, yaw, first = false) {
    ...
    // Nothing braces the weapon — there is no aimed stance to brace in — so the
    // kick is always at full travel. The first shot of a burst kicks harder.
  ...
  update(dt, s) {
    ...
    // The hip pose is the ONLY pose. No sprint carry, no low ready, and above
    // all no solved ADS pose — the weapon sits where it sits and the crosshair
    // is in the HUD.
    ...
    const bobAmt =
      def.bobScale * clamp01(speed / 4.2) * (s.airborne ? 0.25 : 1);
    if (speed > 0.05) {
      // Stride frequency scales with speed; sprint takes longer strides.
      this.bobPhase += dt * (3.1 + speed * 0.72);
    ...
    /* -------- viewmodel FOV ------------------------------------------- */
    // Fixed. The viewmodel camera used to narrow with ADS; with no ADS there is
    // nothing left to drive it, and a viewmodel FOV that moves for any other
    // reason just makes the weapon breathe.
    const VIEW_FOV = 60;
    if (Math.abs(vcam.fov - VIEW_FOV) > 1e-3) {
      vcam.fov = VIEW_FOV;
