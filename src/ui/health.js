...
export class HealthFx {
  ...
  /**
   * Hide the vitals widget (not the hurt overlays, which live on their own
   * layer and are already zeroed by the state). A health bar reading 0/100 is
   * information a dead player has no use for, and it competes with the one
   * thing they do need to read — who is left.
   */
  setVisible(v) {
    setStyle(this.vitals, 'display', v ? '' : 'none');
  }
